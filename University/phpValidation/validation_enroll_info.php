<?php
	$fname = $lname = $id  = $gender = $enrolledcourse = $section =  $email = "";
	$err_fname = $lname = $err_id = $err_gender  = $err_enrolledcourse = $err_section  = "";
	...
	if(isset($_POST["signup"])){
		...
		if(empty ($_POST["enrolledcourse"])){
			$err_enrolledcourse="Course Name Required";
			$hasError = true;
		}
		else{
			$enrolledcourse= htmlspecialchars($_POST["enrolledcourse"]);
		}
		...
		if(!$hasError){
			...
			$user->addChild("enrolledcourse",$enrolledcourse);
